-- ═══════════════════════════════════════════════════════════════
--  Khedmti — تنصيب كامل من الصفر
--  مشروع Supabase جديد → SQL Editor → الصق هذا كامل → Run
--  (ملف واحد يكفي. 01_secure_auth.sql خاص فقط بترقية قاعدة قديمة)
-- ═══════════════════════════════════════════════════════════════
...
create table if not exists agencies (
  ...
  password_hash   text default '',          -- موروث، ما يتستعملش
  pw_hash         text,                     -- bcrypt
  phone           text default '',
  plan            text default 'free',
  account_type    text default 'agency',    -- agency | solo
...
-- كل حالة التطبيق (الفريق، الزبائن، الفيديوهات، الفواتير، المهام)
-- تقعد في jsonb وحدة — أي ميزة جديدة ما تحتاجش migration.
create table if not exists app_data (
  id         uuid primary key default gen_random_uuid(),
  agency_id  uuid unique references agencies(id) on delete cascade,
  data       jsonb default '{"editors":[],"clients":[],"videos":[],"invoices":[],"tasks":[]}'::jsonb,
...
-- ── 12. حساب المدير العام ─────────────────────────────────────
-- 1) سجّل حسابك من المنصة عادي (زر "سجل وكالتك مجاناً")
-- 2) رجع لهنا، بدّل الإيميل وشغّل:
--
--    update agencies set is_super = true
--     where email = 'انت@الايميل.com';
--
-- ما تكتبش كلمات السر هنا — api_register تخدمها bcrypt وحدها.
